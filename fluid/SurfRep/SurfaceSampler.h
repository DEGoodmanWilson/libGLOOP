...
public:
  ...
  /*DEG not used yet*/
  __inline Vector getColorAt(const Vector& v0) const;
...
__inline Vector SurfaceSampler::getColorAt(const Vector& v0) const
{
  Vector returnVal(0.0, 0.0, 0.0);
  assert(surfRep != 0);
    ...
    for (int y=locMin[1]; y<=locMax[1]; y++)
    {
      for (int z=locMin[2]; z<=locMax[2]; z++)
      {
        for (node = buckets->find(x,y,z); node != 0; node = node->next)
        {
          ...
          if (distSquared < node->strength)
          {
            returnVal += (node->color / distSquared) - Vector(1.0f, 1.0f, 1.0f);
          }
        }
      }
    }
...
//            returnVal += (1.0f / (distSquared*distSquared)) - 1.0f;
            returnVal += (node->strength / distSquared) - 1.0f;
          }
