...
//    const float lightColor[3] = {255.0f, 255.0f, 255.0f};
    ...
    for (unsigned int x=0; x<colors.size(); x+=4)
    {
      Vector normal(normals[x+0], normals[x+1], normals[x+2]);
      ...
      if (dotProd > 0)
      {
        colors[x+0] += (unsigned char)Math::minOf(255-ambient[0], (int)(lightColor[0]*dotProd));
        colors[x+1] += (unsigned char)Math::minOf(255-ambient[1], (int)(lightColor[1]*dotProd));
        colors[x+2] += (unsigned char)Math::minOf(255-ambient[2], (int)(lightColor[2]*dotProd));
      }
    }
  }
}

void ImplicitMesh::doSphereMap(const Position cameraPosition)
{
  if (getUseTexCoordArrays())
  {
    texCoords.resize(vertices.size()/2);
    const Position& camPos = cameraPosition;
    
    for (unsigned int x=0; x<texCoords.size(); x+=2)
    {
