...
#undef DRAW_CORNER
  ...
  if (!shouldRecurseOnCube(points,surfaceThreshold))
  {
    return;
  }
...
//                                    {0,0,1}, {1,0,1}, {1,1,1}, {0,1,1} };
  ...
  {
    int putBreakpointHere;
    putBreakpointHere = 0;
  }

  getCube()->setCurrentCube(downLoc[0], downLoc[1], downLoc[2], size>>1);
  recursiveTessellate(points, downLoc, size>>1,surfaceThreshold);
...
int RecursiveTessellator::determineBoundingBox(const PotentialPoints& points, float cubeletSize)
{
  ...
  int numCubeletsX = (int)((maxX - minX) / cubeletSize);
  int numCubeletsY = (int)((maxY - minY) / cubeletSize);
  int numCubeletsZ = (int)((maxZ - minZ) / cubeletSize);

  int largestNumCubelets = (int)Math::maxOf(numCubeletsX, Math::maxOf(numCubeletsY, numCubeletsZ));
  
  // Round up to next power of two.
  int logNextHighestPowerOfTwo = (int)ceil(log((float)largestNumCubelets) / log(2.0f));
