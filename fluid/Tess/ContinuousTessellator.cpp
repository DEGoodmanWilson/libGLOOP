...
ContinuousTessellator::~ContinuousTessellator(){}
...
void ContinuousTessellator::walkSurfaceFromPoint(const PotentialPoints& points, int startingPoint, const Vector& minCorner, float cubeletSize, float surfaceThreshold)
{
  ...
  cubeLoc[0] = (unsigned short)floor((point.x-minCorner.x) / cubeletSize);
  cubeLoc[1] = (unsigned short)floor((point.y-minCorner.y) / cubeletSize);
  cubeLoc[2] = (unsigned short)floor((point.z-minCorner.z) / cubeletSize);
