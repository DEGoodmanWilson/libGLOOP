...
IntHash<Value>::IntHash()
{
  ...
  int packSize;
  packSize = sizeof(HashPack<Value>);
...
void IntHash<Value>::drawDebugGraph(int x0, int y0, int x1, int y1)
{
  ...
  {
    int curLoc = (int)(x * getNumHashBins() / ((float)barRange));
    int nextLoc = (int)((x+1) * getNumHashBins() / ((float)barRange));

    nextLoc = (int)Math::maxOf(nextLoc, getNumHashBins());
