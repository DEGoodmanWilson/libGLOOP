...
int TextureManager::addTexture(const std::string& rawFile, int w, int h, bool alpha, bool repeat)
{
  ...
  {                                   
    ...
    {
      int newW = (int)Math::minOf(w, maxTextureSize);
      int newH = (int)Math::minOf(h, maxTextureSize);
    ...
    else
    {
      ...
      {
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, OpenGL::CLAMP_TO_EDGE);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, OpenGL::CLAMP_TO_EDGE);
      }
      else
      {
...
void TextureManager::deleteTextures()
{
  ...
  for (unsigned int x=0; x<proceduralTextures.size(); x++)
  {
